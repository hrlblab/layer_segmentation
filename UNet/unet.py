...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        # "model_path"    : 'logs/ep050-loss0.155-val_loss0.212.pth',
    ...
    def __init__(self, **kwargs):
        ...
        if self.num_classes <= 21:
            ...
            self.colors = [
                            [0, 0, 0],     # barckground
                            [255, 0, 255], # inner medulla
                            [0, 255, 255], # inner stripe
                            [0, 255, 0],   # outer stripe
                            [255, 0, 0]    # cortex
                        ]
        else:
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        if self.mix_type == 0:
            ...
            #     seg_img[:, :, 2] += ((pr[:, :] == c ) * self.colors[c][2]).astype('uint8')
            seg_img = np.reshape(np.array(self.colors, np.uint8)[np.reshape(pr, [-1])], [orininal_h, orininal_w, -1])
            image_rgb_seg = Image.fromarray(np.uint8(seg_img))
            image_rgb = Image.blend(old_img, image_rgb_seg, 0.7)
            #------------------------------------------------#
            #   将新图片转换成Image的形式
            #------------------------------------------------#
            # image   = Image.fromarray(np.uint8(seg_img))
            image = Image.fromarray(np.uint8(pr))
            image_rgb = Image.fromarray(np.uint8(image_rgb))
